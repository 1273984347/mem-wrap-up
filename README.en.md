...
- **Tools**: file search (Grep/Read) + shell (PowerShell examples; macOS/Linux use bash/zsh equivalents — see the cross-platform command table in SKILL.md); subagent/task spawning is **optional** — without it, the skill falls back to the degradation mode (see the "no-subagent degradation mode" section in SKILL.md).
